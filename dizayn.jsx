...
const LentaScreen = () => {
  const orders = [
    {
      id: 1,
      cargo: "Konditsionerlar",
      weight: 8.5,
      price: "3,200,000",
      truckType: "Refrijerator",
      waypoints: [
        {address: "Toshkent, Sergeli daxasi", type: "PICKUP"},
        {address: "Jizzax shaxri", type: "TRANSIT"},
        {address: "Buxoro, Markaziy ombor", type: "DELIVERY"}
      ]
    },
    {
      id: 2,
      cargo: "Elektronika (Maishiy)",
      weight: 2.1,
      price: "12,800,000",
      truckType: "Tent (Yopiq)",
      waypoints: [
        {address: "Urumchi, Xitoy", type: "PICKUP"},
        {address: "Toshkent, Bojxona", type: "TRANSIT"},
        {address: "Nukus, Qoraqalpog'iston", type: "DELIVERY"}
      ]
    }
  ];
    ...
    <div className="px-5 pt-4 pb-32 space-y-4 overflow-y-auto h-full no-scrollbar">
      <div className="flex items-center justify-between mb-2">
        <div>
          ...
          <p className="text-[10px] text-white/30 font-bold uppercase tracking-widest mt-1 italic">Jonli efir: 24 ta faol e'lon</p>
        </div>
        ...
        <GlassCard key={order.id} className="p-6 group border-white/5 hover:border-purple-500/30">
          <div className="flex justify-between items-start mb-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="font-black text-lg tracking-tight">{order.cargo}</h3>
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-[9px] uppercase tracking-widest font-black text-purple-400 bg-purple-400/10 px-2 py-0.5 rounded border border-purple-400/20">{order.truckType}</span>
                  <span className="text-[9px] uppercase tracking-widest font-black text-white/30">{order.weight} Tonna</span>
                </div>
              </div>
            </div>
            <div className="text-right">
              <p className="text-emerald-400 font-black text-2xl tracking-tighter">{order.price.split(',')[0]}k</p>
              <p className="text-[9px] opacity-40 uppercase font-black tracking-widest">uzs</p>
            </div>
          </div>

          <div className="mb-6 px-1">
            <RouteVisualizer waypoints={order.waypoints} />
          </div>

          <div className="flex gap-3 mt-4">
            <button className="flex-[2.5] py-4 rounded-2xl bg-white text-black font-black text-sm active:scale-95 transition-all shadow-[0_10px_20px_rgba(255,255,255,0.1)] uppercase tracking-tighter">
              Band qilish
            </button>
            <button className="flex-1 py-4 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center gap-2 text-white/80 font-black text-[11px] active:scale-95 transition-all uppercase tracking-tighter hover:bg-white/10">
...
const MapScreen = () => {
  ...
  const mockPins = [
    { id: 1, x: 30, y: 40, price: "3.2M", cargo: "Konditsioner", from: "Toshkent", to: "Buxoro", profit: "Yuqori", eta: "5s 30d" },
    { id: 2, x: 70, y: 25, price: "12.8M", cargo: "Elektronika", from: "Xitoy", to: "Nukus", profit: "Premium", eta: "12s 10d" },
  ];
    ...
    <div className="px-5 pt-4 pb-32 h-full overflow-y-auto no-scrollbar space-y-6">
      <div className="flex flex-col gap-5">
        <div className="flex justify-between items-end">
          ...
          <span className="text-[10px] font-black text-purple-500 uppercase tracking-widest bg-purple-500/10 px-3 py-1 rounded-full border border-purple-500/20">1 Faol</span>
        </div>
              ...
              <div className="flex justify-between items-start mb-4">
                <div>
                  <div className="flex items-center gap-2 mb-1.5">
                    ...
                    <span className="text-[9px] font-black text-white/40 uppercase tracking-widest">{order.id}</span>
                  </div>
                  <h3 className="text-lg font-black">{order.cargo}</h3>
                </div>
                <div className="text-right">
                  <p className="text-lg font-black text-emerald-400">{order.income.split(',')[0]}<span className="text-[10px] opacity-40 ml-1">K</span></p>
                </div>
              </div>
                ...
                <div className="flex justify-between items-end">
                   <div className="flex items-center gap-3">
                      <div className="flex flex-col">
                         <span className="text-[8px] opacity-30 font-black uppercase">Tezlik</span>
                         <span className="text-xs font-black">{order.speed}</span>
                      </div>
                      <div className="flex flex-col">
                         <span className="text-[8px] opacity-30 font-black uppercase">ETA</span>
                         <span className="text-xs font-black">{order.eta}</span>
                      </div>
                   </div>
                   <span className="text-[10px] font-black text-purple-400">{order.progress}%</span>
                </div>
                <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
                  <div className="h-full bg-gradient-to-r from-purple-600 to-indigo-500 shadow-[0_0_15px_rgba(168,85,247,0.5)]" style={{ width: `${order.progress}%` }} />
                </div>
              </div>
            </div>
      ...
      <div className="pt-6">
        <div className="w-24 h-24 rounded-3xl bg-gradient-to-tr from-purple-600 to-indigo-600 mx-auto mb-4 flex items-center justify-center p-1 shadow-2xl">
          <div className="w-full h-full bg-[#050610] rounded-[1.4rem] flex items-center justify-center overflow-hidden">
            <img src="https://api.dicebear.com/7.x/avataaars/svg?seed=Felix" alt="Avatar" className="w-20" />
          </div>
        </div>
        <h2 className="text-2xl font-black tracking-tight">Azizbek Yakubovich</h2>
        <p className="text-[10px] font-black text-white/30 uppercase tracking-[0.2em] mt-1 italic">Professional Driver</p>
      </div>
